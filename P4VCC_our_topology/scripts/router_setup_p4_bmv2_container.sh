...
#docker exec -w ~/P4-VCC/exercises/mri_port_forward fabric_p4 sh -c 'p4c --p4runtime-files basic_tunnel.txt --target bmv2 --arch v1model mri.p4 '

if [ "$#" -eq 2 ]
    then
        docker exec -d -it fabric_p4 sh -c 'simple_switch --interface 1@'${port1_iface}' --interface 2@'${port2_iface}' /root/tutorials/exercises/basic_tunnel/basic_tunnel.json'  
        echo 'simple_switch --interface 1@'${port1_iface}' --interface 2@'${port2_iface}' /root/tutorials/exercises/basic_tunnel/basic_tunnel.json'
fi
...
if [ "$#" -eq 3 ]
    then
        ...
        echo 'simple_switch --interface 1@'${port1_iface}' --interface 2@'${port2_iface}' --interface 3@'${port3_iface}' /root/tutorials/exercises/basic_tunnel/basic_tunnel.json'
fi

if [ "$#" -eq 4 ]
    then
        docker exec -d -it fabric_p4 sh -c 'simple_switch --interface 1@'${port1_iface}' --interface 2@'${port2_iface}' --interface 3@'${port3_iface}' --interface 4@'${port4_iface}' /root/tutorials/exercises/basic_tunnel/basic_tunnel.json'  
        echo 'simple_switch --interface 1@'${port1_iface}' --interface 2@'${port2_iface}' --interface 3@'${port3_iface}' --interface 4@'${port4_iface}' /root/tutorials/exercises/basic_tunnel/basic_tunnel.json'
fi
